...
CFLAGS :=  -qno-opt-dynamic-align -fp-model precise -std=gnu99 -O0 -g -qopt-report -xCORE_AVX2 -no-fma
CMAKE_OPTS :=  -DPIO_ENABLE_LOGGING=ON
CPPDEFS := $(CPPDEFS)  -DCESMCOUPLED -DFORTRANUNDERSCORE -DCPRINTEL
CXX_LDFLAGS :=  -cxxlib
CXX_LINKER := FORTRAN
FC_AUTO_R8 := -r8
FFLAGS :=  -qno-opt-dynamic-align  -convert big_endian -assume byterecl -ftz -traceback -assume realloc_lhs -fp-model source -O0 -g -check uninit -check bounds -check pointers -fpe0 -check noarg_temp_created -qopt-report -xCORE_AVX2 -no-fma
FFLAGS_NOOPT := -O0
FIXEDFLAGS := -fixed
FREEFLAGS := -free
